# Coder

[TOC]

## Dev

[stackoverflow]("全球最为活跃的程序员技术问答交流社区")

[LintCode]("刷算法")

[DZone](https://www.dzone.com "分享知识社区")

[Bytes]("Bytes是面向开发人员和IT专业人员的交流社区")

[Github]("")

[Hacker News](https://news.ycombinator.com/news)

[hongkiat](https://www.hongkiat.com/blog/)

[Reddit_programming](https://www.reddit.com/r/programming/)

[InfoQ](https://www.infoq.com/)

[daniweb]

[sitepoint](https://www.sitepoint.com/ "WEB开发领域")

[tutorialspoint](https://www.tutorialspoint.com/index.htm "众多在线免费教程内容")

[Google developers]()

[DEV](https://dev.to/"开发者社区网站，站内可以看到全球各地的资讯和经验分享。")

[codeproject]()

[zentaoPM]()

[Bitbucket]()

[MDN Web Docs]("学习 Web 开发的实践平台")

[SimilarSites]("输入该网站的地址，SimilarSites会为你检索相似的网站")

[Semantic Schoolar]("免费学术搜索引擎")

[Stackshare]("不知道如何根据应用场景选择技术栈，又没有时间可以去了解技术框架时，可以在这里查找相似产品的技术栈。")

[Internet Archive]("海量免费资源")

[Hashnode]("Hashnode是一个免费的内容创建平台和社区")

[vogella](https://vogella.com/ "Training and education")

[PHP](https://www.php.com/)

## News，blog

[The Verge]("")

Engadget

Gizmodo

TechCrunch

BusinessInsider

CNET

ZDNet

Lifehacker

Mashable Tech

ReadWrite

Businessweek

The Next Web

## Tec blog

(https://developer.ibm.com/)

(https://www.oracle.com/java/technologies/java-se-glance.html)

[IT-ebooks](https://it-ebooks.info/)

(http://stackoverflow.com/questions/tagged/java)

## Java&&Android

[Appbrain](https://www.appbrain.com/ "Everything you need for a successful Android app")

[programcreek](https://www.programcreek.com/ "Programme Examples")

[javacodegeeks](https://www.javacodegeeks.com/)

[tutorialspoint_Java](https://www.tutorialspoint.com/java/)

[f-droid](https://f-droid.org/ "Android上的自由和开放应用集合")

[Java各版本向导](https://mkyong.com/)

[知乎推荐程序员网站](https://www.zhihu.com/question/26155575)

## Google 搜索返回的 PHP 教程一半含有 SQL 注入漏洞

[固点转载](https://www.solidot.org/story?sid=68375)

[16 of 30 Google results contain SQL injection vulnerabilities](https://waritschlager.de/sqlinjections-in-google-results.html)

[Simple User Registration With Email Verification In PHP MYSQL](https://code-boxx.com/user-registration-email-php-mysql/)

[Google results for PHP tutorials contain SQL injection vulnerabilities](https://news.ycombinator.com/item?id=27952135)

## 国外学习网站

[Coursera]

[Udemy]

edx

**可汗学院**

open Yale Course

MIT openCourseWare

OCW

AdobeKnowHow

Professional Grade Ruby on Rails Online ？ootcamp

Udactiy

thenewboston

Coursetalk

codeschool

video2Brain

NovoED

openHPI

No Excuse List

Futrelearn

Open2study

Academic Earth

BetterExplained

mathispower4u

TED

Echo360

Smashing Magazine

Grovo

OpenCulture

creativelive

saylor

Crafsy

Knewton

中国大学MOOC

网易公开课

TEDtoChina

[学习网站收藏站点](http://1nami.com/)

## 自学编程的误区

*[CodeSheep程序羊 自学编程的致命误区](https://www.youtube.com/watch?v=OUHVxCCH_LM)*

1. 重在夯实基础，基础概念、基础路线。时髦的东西基于基础的东西，时髦的自然淘汰的快。
2. 学习最好的编程语言。这是个伪命题。语言是生产力工具，语言有适用的范畴，第一种语言
总是最难的，之前的语言学习会成为后面的基础。当代建议从脚本语言如Python起步，Java&C++属于深入型的。
3. 交流式学习，找人帮忙，良好的提出问题，解决问题。适合交流的网站SF、ST在、Github，博客51CTO、CSDN，社交APP。观察下来比较难的，没人愿意为他人花费太多时间、精力。好在博客类的长存信息。交流发现自己的不足，特别式盲点和弱点。
4.  代码记不住？代码不是记忆的。设计模式，流程需要记忆之外，代码本身是理解中记忆的，运用中掌握的。有些代码编写的差距来自经验，眼界，靠积累和推荐看大佬的代码和书（Cookbook系列）。学得快，心态自然。
5.  学习系统性。资源精简，先确定路线。
6.  学了就忘。需求驱动，用着熟练。记录、总结、思考、提炼、回顾。理解东西再次学习快。
7.  编程的门槛很低，转专业的很多。

## Python应用领域和竞争力的看法

1. 人工智能：学历门槛高，也就是计算科学基础知识，要求逻辑性很强。
2. 爬虫：反爬趋势，阻止大过爬取，互联网围墙花园，比如微信公众号文章。
3. 后端服务：很少商业采用。
4. 自动化测试、自动化运维：基础工作。
5. 数据分析、算法：同样高端。

绝大部分公司做的是信息化系统，管理类、支付类等等。Java的竞争力强的多。

## 自学编程心得

软件开发是个系统工程，借鉴学习路径和方法。

1. 自学实战挑战。不要像无头苍蝇乱撞，需要权威正确的路径，蓝图制定。
2. 无书。刚开始学视频的时候建议大家学习视频教程，中后期编程核心思想，设计模式，缺少不了看书。
3. 英语，Document，API。
4. 加入社区。见贤思齐，加入社区大牛，教程、代码、对应问题的处理、了解行业信息。Podcast（播客），获取信息和看法。邮件订阅周报（Weekly）。
5. 练习、实践。注重项目的实用性，给你带来了成就感，学以致用。工具链等不是一次性学会的。
6. 另辟蹊径。作出和别人不同的地方，利用前沿科技如机器学习，图形学。

### 多媒体设计师学编程

1. 初期作为新手，应该找能够快速上手的，短期见到成果的，这个符合人性。
2. HTML、CSS、JS前端，PHP后端，W3school.com.cn。
3. 工作要饱和与上班要摸鱼

### SchelleyYuki

1. 路线图介绍
2. 课程推荐[Udacity: Learn the Latest Tech Skills; Advance Your Career](https://www.udacity.com)
3. 


### Hardware

Raspberry Pi is a cute little single-board computer.

architecture is the same.